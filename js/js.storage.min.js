...
var storage = Storages.initNamespaceStorage('auctx').localStorage;
var userData = {};
userData.load = function (keys) {
    if(!keys)keys = storage.keys();
    if(typeof keys === 'string'){
        keys = keys.split(",");
    }
    for(var i=0; i<keys.length; i++){
        var key = keys[i];
        userData[key] = storage.get(key);
    }
...
userData.save = function (keys) {
    if(typeof keys == "string"){
        var dataParts = keys.split(",");
        keys = {};
        for (var i = 0; i < dataParts.length; i++) {
            var prop = dataParts[i];
            keys[prop] =  userData[prop];
        }
    }
    for(var prop in keys){
        storage.set(prop, keys[prop]);
        userData[prop] = keys[prop];
    }
    return userData;
}